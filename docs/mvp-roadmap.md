...
| ↳ Tier history window cap | `infrastructure-fovp` | ✅ Shipped 2026-05-06 |
| ↳ Per-channel severity routing (Pro+) | `infrastructure-dxkk` | ✅ Shipped 2026-05-06 |
| ↳ Cadence split (Starter daily digest / Pro+ realtime) | `infrastructure-xzuz` | ✅ Shipped 2026-05-07 — Vercel cron 0 9 * * * UTC, lib/digest.ts |
| ↳ CSV export (Team) | `infrastructure-0sco` | ✅ Shipped 2026-05-07 — GET /api/account/export.csv, Bearer auth added |
...
- **2026-05-07 (azn9 EPIC closed — 9/9 sub-beads, staging fully smoked)** — All Pro/Team tier-gate features shipped and smoked. xzuz: cadence split (Starter daily digest 9am UTC via Vercel cron, Pro/Team realtime) — lib/digest.ts + sql/010_xzuz.sql + vercel.json. 0sco: GET /api/account/export.csv (Team, cookie + Bearer). cin7: multi-user org seats — organizations + memberships + invitations tables (sql/011_cin7.sql), lib/invite-token.ts, lib/organizations.ts, /api/account/invite, /api/accept-invite, /account/team page + invite-form.tsx; LABELWATCH_INVITE_TOKEN_SECRET in Vercel. yo7k: custom alert rules — alert_rules table (sql/012_yo7k.sql), lib/custom-rules.ts evaluateCustomRules + matchCustomRuleCandidates, /account/rules page; delivery_jobs.match_reason += custom_rule. 2mkx: Team REST API + API keys — api_keys table (sql/013_2mkx.sql, SHA-256 hashed soft-revoke), lib/api-key.ts, GET /api/v1/matches (Bearer auth, pagination, ?from/?to/?severity), export.csv Bearer auth, /account/apikeys page. Bug fixed during smoke: listOrgMembers ambiguous PostgREST FK join fixed with !customer_id hint. NEXT ACTION: revert exje (app/page.tsx TIERS filter + /api/checkout server-side rejection) → Pro/Team purchasable.
